...
- Finished visual design, premium/branded look, rebrand, or any non-blank prompt-driven site: use `skills/qlander-design-research/SKILL.md` to source and approve a direction, then `skills/qlander-design/SKILL.md`. Persist colors/radius in `data/theme.json`, typography/spacing/layout/component policy in `data/design-system.json`, and material page/section renderer changes in `src/layout-handoffs.ts` plus `qlander.manifest.json.design`. A recolored or repopulated starter is not a finished prompted design.
