...
- Finished visual design, premium/branded look, rebrand, or any non-blank prompt-driven site: use `skills/qlander-design-research/SKILL.md` to capture screenshot evidence and approve a direction, then `skills/qlander-design/SKILL.md`. Research produces a `reference-manifest.json` and a `layout-blueprint.json` under `.qlander/design-research/<run-id>/`; design builds the blueprint's renderer under `src/design/<direction-slug>/`, gets the static silhouette approved, and only then writes final copy into its slots. Persist colors/radius in `data/theme.json`, typography/spacing/layout/component policy in `data/design-system.json`, and material page/section renderer changes in `src/layout-handoffs.ts` plus `qlander.manifest.json.design`. A recolored or repopulated starter is not a finished prompted design, and neither is a bundled `src/design-variants/*` renderer.
...
- New project initialization: use `pnpm qlander:init` with a profile and creation intent. Prompted mode is the default for any supplied brief/instruction and must finish with approved research, a shared design system, and a material layout handoff. Use `--blank` only when the user explicitly asks for the built-in starter as the initial result. Marketing sites accept `--no-blog`, `--no-products`, and `--no-resources`; `--minimal` applies all three exclusions.
