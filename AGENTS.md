...
- Finished visual design, premium/branded look, rebrand, a design built elsewhere, or any non-blank prompt-driven site: use `skills/qlander-design/SKILL.md`. Port an existing design with its `references/port-recipe.md`, or propose one and record the direction per `references/design-direction.md`. A `design-spec.json` from the sibling `qlander-design` repository is the short path: its `theme` and `designSystem` copy in verbatim and its `handoffs` seed both registrations. Persist colors/radius in `data/theme.json`, typography/spacing/layout/component policy in `data/design-system.json`, and material page/section renderer changes in `src/layout-handoffs.ts` plus `qlander.manifest.json.design`. A recolored or repopulated starter is not a finished prompted design.
