...
- The profile ships demo `/blog`, `/products`, and `/resources` routes. To start without the blog or products demos, pass `--no-blog`, `--no-products`, or `--minimal` (both) to `pnpm qlander:init`; init then removes the selected route files, empties the collection, and cleans navigation, manifest, route-seo, edit-map, and any content-page links to the dropped routes. This is the preferred way to exclude those demo routes, done once at creation rather than as a later developer-mode edit.
