...
export const PageSectionSchema = z.discriminatedUnion("type", [
  z.object({ id: z.string().min(1), type: z.literal("hero"), headline: z.string().min(1), subheadline: z.string().min(1), primaryCta: CtaSchema, secondaryCta: CtaSchema.optional(), image: MediaSchema.optional(), imagePromptId: ImagePromptIdSchema.optional() }).strict(),
  z.object({ id: z.string().min(1), type: z.literal("featureGrid"), headline: z.string().min(1), items: z.array(z.object({ title: z.string().min(1), description: z.string().min(1), image: MediaSchema.optional(), imagePromptId: ImagePromptIdSchema.optional() }).strict()).min(1) }).strict(),
