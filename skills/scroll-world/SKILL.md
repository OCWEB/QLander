...
name: scroll-world
description: >
  Build an immersive scroll-scrubbed "fly through the world" landing page or scoped
  page section for any industry or brand. As the visitor scrolls, a pre-rendered camera
  ...
  story beats/sections, and brand kit, then prepares cohesive scenes + seamless camera
  clips through a default manual slow queue or an explicitly approved API path and wires
  a portable, framework-agnostic scroll-scrub engine.
  Use when the user wants a "3D world" / "browse-through-the-industry" hero, an inline
  scroll-section, a scroll cinematic, a diorama landing, or to turn a business into a
  scrollable world.
---
...
cuts. The visuals are AI-generated with a compatible image/video provider; the page just scrubs pre-rendered video
...
The value of this skill is the frame-locked pipeline, the prompts, and the seam method —
not the framework.

---

## Step 0 — Choose generation mode and bootstrap

1. **Default: manual queue.** Read `references/manual-queue.md`. Prepare `queue.md` and
   machine-readable `queue.json`
   and let the user render through their account's slow/relax queue. Do not install a
   provider CLI, call a generation API, or spend credits unless the user explicitly
   chooses the automated path. Provider availability and account terms still apply.
2. **Automated/API alternative.** If the user explicitly chooses it and `higgsfield`
   is not on `$PATH`, install per the
...
0. **Delivery context — decide before scene generation.** If the current repository is a
   QLander site, use `references/qlander-adapter.md`. Choose `scroll-section` when the
   sequence belongs between normal sections on an existing page; choose a named route
   such as `/tour` when it needs its own destination. Keep all normal pages intact. If
   the approved experience owns `/`, use QLander's
   `root-scroll-world` initialization profile for a new standalone microsite or the
   adapter's explicit root registration for an existing repository. Never build a
   standalone page and retrofit it later.
...
5. **Mobile version — ALWAYS ask this; never silently generate both.** Ask as a
   ...
   *"Want a mobile-optimized version too? The mobile version is a second camera chain
   ...
   landscape film — which roughly doubles the number of manual render jobs or API
   generations."*
   Options: "Desktop only" / "Desktop + mobile (native 9:16 — ~2× render jobs)".
   State the API credit estimate only when the automated path was selected.
...
6. **Generation mode — default to manual queue.** Present manual slow queue first and
   select it when the user has no preference. It creates files for the user to render
   manually and makes no generation API calls. Offer automated/API generation only as an
   explicit alternative. For that alternative, present the render tiers, then compute
   and state the estimated total for
   ...
   - **Manual queue (default, no generation API calls).** The agent writes a two-phase
     prompt `queue.md` plus synchronized machine-readable `queue.json`. The user renders by hand in Magnific's slow/relax queue, hands the files
     back in a `results/` folder, and the agent does only local work (encode, frame
     extraction, seam QA, walkthrough, wiring) via `references/scripts/`. Same prompts,
     no QLander generation API calls — connectors still frame-lock (Phase 1 dives → agent
     extracts boundary frames → Phase 2 connectors use them as keyframes). Full workflow
     + filename contract in `references/manual-queue.md`; queue format in
     `references/queue-template.md` and `references/queue-template.json`. When chosen, skip the cost calibration / budget gate
     below. Do not promise that the provider account is free; only assert that QLander
     does not call a generation API or spend API credits in this mode.
...
These stills double as **eager video posters and fallback images**, so keep them. The
engine loads every poster eagerly because fixed, initially transparent layers may never
trigger browser lazy-loading before their video is ready.
...
**QLander route or section:** read and follow `references/qlander-adapter.md`. Use the
bundled `qlander:experience` command. Named and root experiences use the shared
`ScrollWorldPage` renderer. A `scroll-section` uses `ScrollSection` and the same engine
with `mode: "section"`, which scopes progress to a sticky viewport inside the page.
Do not copy `index-template.html`, generate a one-off route component, or add a custom
teaser section type. Put approved media under
`public/experiences/<slug>/`, populate `data/experiences/<slug>.json`, and link to the route
from an existing QLander CTA or navigation item only when approved.

**Standalone page or non-QLander project:** use the portable assembly below.
...
smoothing, eager poster loading, blob loading, lazy prefetch of nearby clips,
frame-matched crossfades, pinned
...
- `references/scrub-engine.js` — the portable, config-driven scrub engine (builds DOM +
  injects CSS; eager posters, blob-seek, lazy clip loading, aligned route dots, seam
  crossfade, copy, route rail, reduced-motion, and
...
- `references/manual-queue.md` — the default no-generation-API path: emit a prompt queue,
  user renders in Magnific's slow queue, and the agent assembles from `results/`.
- `references/qlander-adapter.md` — register Scroll World as a dedicated route or scoped
  `scroll-section` inside a multi-page QLander site without replacing the site.
- `references/queue-template.md` — the fillable two-phase `queue.md` structure.
- `references/queue-template.json` — machine-readable manual-queue status and dependencies.
- `references/scripts/` — local assembly (no API): `ingest.sh` (encode + extract
  boundary frames, desktop + mobile), `seam-qa.sh` (SSIM seams), `walkthrough.py`
  (crossfaded preview), `register-qlander-experience.mjs` (safe QLander route setup).
