/* ============================================================================
   ...
   USAGE
     mountScrollWorld(document.getElementById('world'), {
       mode: 'route',      // use 'section' for a sticky experience inside a normal page
...
function mountScrollWorld(container, config) {
  const sectionMode = config.mode === 'section';
  ...
  if (sectionMode) container.classList.add('sw-root--section');
  ...
  SEGMENTS.forEach(s => {
    ...
    // Posters load eagerly: they are the no-JS/pre-clip fallback and sit in
    // position:fixed, opacity:0 layers where Chrome's lazy-load never fires
    // (leaving the scene blank until the clip paints). They're small; load them now.
  ...
  SECTIONS.forEach((s, i) => {
    ...
    c.innerHTML =
      ...
      (s.title ? `<${config.headingLevel === 'h1' && i === 0 ? 'h1' : 'h2'} class="sw-copy__title">${esc(s.title)}</${config.headingLevel === 'h1' && i === 0 ? 'h1' : 'h2'}>` : '') +
  ...
  let vh = window.innerHeight, stageX = 0, totalW = 0, sectionStart = 0, activeIndex = -1, ticking = false;
  let laidOutW = window.innerWidth;   // width the current layout was computed at (see onResize)

  function layout() {
    vh = sectionMode ? (viewport.getBoundingClientRect().height || window.innerHeight) : window.innerHeight;
    laidOutW = window.innerWidth;
    stageX = window.innerWidth > 860 ? 4 : 0;
    let off = 0;
    SEGMENTS.forEach(s => { s.start = off * vh; off += s.w; s.end = off * vh; });
    totalW = off;
    track.style.height = (sectionMode ? totalW * vh : totalW * vh + vh) + 'px';
    sectionStart = (window.scrollY || window.pageYOffset) + container.getBoundingClientRect().top;
    read();
  }

  function jumpTo(i) {
    const seg = SECTIONS[i]._seg;
    const offset = sectionMode ? sectionStart : 0;
    window.scrollTo({ top: offset + seg.start + (seg.end - seg.start) * 0.5, behavior: reduce ? 'auto' : 'smooth' });
  }
  ...
  function read() {
    ...
    if (sectionMode) sectionStart = pageY + container.getBoundingClientRect().top;
    const rawY = sectionMode ? pageY - sectionStart : pageY;
    const y = sectionMode ? clamp(rawY, 0, totalW * vh) : rawY;
...
function injectCSS() {
  ...
  .sw-route__dot{position:relative;border:0;padding:0;background:transparent;cursor:pointer;width:14px;height:14px;display:grid;place-items:center;}
  ...
  .sw-root--section{position:relative;isolation:isolate;}
  .sw-root--section .sw-viewport{position:sticky;top:0;display:block;height:100vh;height:100svh;overflow:hidden;background:var(--sw-bg);}
  .sw-root--section .sw-sky,.sw-root--section .sw-scrollbar,.sw-root--section .sw-stage,.sw-root--section .sw-copylayer,.sw-root--section .sw-route,.sw-root--section .sw-hint{position:absolute;}
  .sw-root--section .sw-topbar{display:none;}
  .sw-root--section .sw-sky,.sw-root--section .sw-stage,.sw-root--section .sw-copylayer{inset:0;}
  .sw-root--section .sw-scrollbar{top:0;left:0;right:0;}
